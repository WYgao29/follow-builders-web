...
网页首先请求中文数据仓的 `data/index.json`，再按设置读取最近 7、14 或 30 个 `data/days/YYYY-MM-DD.json` 日分片。日分片是 v3 扁平数组，按推文 `id`、播客 `guid`、博客 `url` 去重；重复项保留内容和总结更完整的版本。发布切换期间加载器仍兼容只读 v2 分片。
...
浏览器缓存键为 `fb.web.v6`。网页显示数据中已有的 `summaryZh` 中文总结与英文原文；从 v2 读取的旧翻译字段会在进入内存和缓存前删除。

如需暂停中文总结显示，将 `data-core.js` 中的 `AI_SUMMARIES_VISIBLE` 设为 `false` 即可隐藏三类内容的总结。

## 功能

- 单日时间线与最近 7/14/30 日切换
- X 推文、播客和博客的中文 AI 简述 + 英文原文阅读
...
├── about.html              # 信息源介绍
├── style.css               # 移动端优先样式
├── data-core.js            # 可测试的 v2/v3 过渡契约、加载与安全 URL 核心
...
网页代码采用 [MIT License](LICENSE)。聚合的推文、播客、博客内容版权归原作者或相应权利人所有；本仓库许可证不授予对聚合内容的额外权利。
